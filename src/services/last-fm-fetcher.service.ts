...
export class LastFmFetcherService {
    ...
    async rediscoverLovedTracks(
        ...
        const stringTimeRange = `${candidateFrom}-${candidateTo}-${comparisonFrom}-${comparisonTo}`

        const time_range_redis = await redis.getBuffer(`lastfm:users:${userlastfm}:${stringTimeRange}`)

        if (time_range_redis) {
            return JSON.parse(zlib.gunzipSync(time_range_redis).toString())
        }
        ...
        const sizeBeforeBytes = Buffer.byteLength(JSON.stringify(lastTimeListened), "utf8")
        const sizeBeforeKB = (sizeBeforeBytes / 1024).toFixed(2)
        const sizeBeforeMB = (sizeBeforeBytes / (1024 * 1024)).toFixed(2)
        console.log(`📊 Lastfm (antes da compressão): ${sizeBeforeKB} KB / ${sizeBeforeMB} MB`)


        const compressedTopMusics = zlib.gzipSync(
            JSON.stringify(lastTimeListened)
        )
        ...
        console.log(`📊 Lastfm (depois da compressão): ${sizeAfterKB} KB / ${sizeAfterMB} MB`)
        console.log(`📊 Número de tracks: ${lastTimeListened.length}`)

        // retornar como string Base64 para o BullMQ não quebrar o binário
        return compressedTopMusics.toString("base64")
    }
}
